...
from gi.repository import Gdk; screen=Gdk.Screen.get_default()
...
DIRECTION_POINTS = [[1 - y * 0.018, x/100, 1-x/100] for y, x in enumerate(range(15, 40))]

def draw_circle_arc(image, index, angles):
    center = (int(image.shape[1]/2), 350)
    return cv2.ellipse(image, center, (int(300 + index/2), int(300 + index/2)), 0, angles[0], angles[1], COLOR_RANGE[index], int(2 + 10 - index/5))
...
def main():
    ...
    car_frame = cv2.resize(
        car_frame, 
        (
            int(car_frame.shape[1]*(700/car_frame.shape[0])),
            700
        ))
    ...
    camera_frame = cv2.resize(
        camera_frame, 
        (
            int(camera_frame.shape[1]*(700/camera_frame.shape[0])),
            700
        ))
    camera_frame = camera_frame[:, \
                                int((camera_frame.shape[1] - 1200 + car_frame.shape[1])/2):  \
                                int((camera_frame.shape[1] + 1200 - car_frame.shape[1])/2)]
    ...
    while key != ord('q'):
        ...
        for obj in local_node.object_dictionary.values():
            ...
            if isinstance(obj, canopen.objectdictionary.ODArray):
                for subobj in obj.values():
                    print(f'  {subobj.subindex}: {subobj.name} = {local_node.sdo[obj.index][subobj.subindex].raw}')
                    colors[subobj.subindex] = int(local_node.sdo[obj.index][subobj.subindex].raw)
        ...
        cv2.imshow("test", cv2.resize(cv2.hconcat([car_frame_aux, camera_frame_aux]), (SCREEN.width, SCREEN.height)))
        key = cv2.waitKey(50)
